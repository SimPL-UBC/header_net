#!/usr/bin/env python3
"""Create cache tensors for header detection."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create header cache tensors")
    parser.add_argument(
        "--dataset-path",
        default=str(cfg.DATASET_PATH),
        help="Path to dataset root (DeepImpact)",
    )
    parser.add_argument(
        "--header-dataset",
    ...
    parser.add_argument(
        ...
        help="Path to detection dictionary (ball + players)",
    )
    parser.add_argument(
        "--output-dir",
...
def main() -> None:
    ...
    print(f"Generated {len(negatives_df)} negative samples")
    ...
    cache_df = create_header_cache(
        ...
        output_dir,
        window,
        cfg.CROP_SCALE_FACTOR,
        cfg.OUTPUT_SIZE,
        cfg.LOW_RES_OUTPUT_SIZE,
        cfg.LOW_RES_MAX_DIM,
    )

    print(f"Created {len(cache_df)} cache samples")
