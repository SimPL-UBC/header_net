...
class ConvModule(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, conv_cfg=None, norm_cfg=None, act_cfg=None):
        super().__init__()
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias=bias)
        self.norm = nn.BatchNorm3d(out_channels) if norm_cfg else nn.Identity()
...
def _load_checkpoint(model, filename, map_location=None, strict=False, logger=None):
    return load_checkpoint(model, filename, map_location, strict, logger)
...
class ResNet3dCSN(ResNet3d):
    ...
    def forward(self, x):
        x = super(ResNet3dCSN, self).forward(x)
